...
* $\frac{g(\hat{\theta}) - g(\theta)}{SE(g(\hat{\theta}))}$ converges to $\mathcal{N}(0,1)$ in distribution, where 
    * $$ SE(g(\hat{\theta})) = \sqrt{\frac{\bigg[\frac{\partial g(\theta)}{g(\theta)}\bigg]^{2}}{nE \bigg(-\frac{\partial^{2} \log f(Y | \theta)}{\partial \theta^{2}}\bigg)}}$$
...
Step 2: Loglikelihood Function
$$
\ell(\theta) = \ln(L(\theta)) = n(\theta + 1) + \theta (\ln(\prod_{i=1}^{n}y_{i}) \\
$$
...
\ell^{\prime\prime} (\theta) \vert_{\hat{\theta}} < 0
$$
